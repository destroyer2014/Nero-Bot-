...
export const command={name:'menu',aliases:['help','comandos'],description:'Muestra todos los comandos.',async execute({sock,msg,chat,sender,instanceType}){const{date,time}=formatDateTime(config.timezone);const mention=`@${jidToNumber(sender)}`;const uptime=Math.floor(process.uptime());const hours=Math.floor(uptime/3600);const minutes=Math.floor((uptime%3600)/60);const type=instanceType==='subbot'||config.instanceType==='subbot'?'Subbot':'Bot principal';const header=[`✦════ < 🖤 ${config.botName.toUpperCase()} > ════⚝`,'',`✦ *Usuario:* ${mention}`,`✦ *Creador:* ${config.creator}`,`✦ *Versión:* ${config.version}`,`✦ *Instancia:* ${type}`,`✦ *Tiempo activo:* ${hours} h ${minutes} min`,`✦ *Enlace:* ${config.website}`,`✦ *Fecha:* ${date}`,`✦ *Hora:* ${time}`,''].join('\n');const body=`${header}${sections.map(({title,entries})=>section(title,entries)).join('\n\n')}\n\n✦════ < ✨ FIN DEL MENÚ > ════⚝`;try{const video=await fs.readFile(path.resolve(projectRoot,config.menuVideo));await sock.sendMessage(chat,{video,gifPlayback:true,caption:body,mentions:[sender]},{quoted:msg})}catch{await sock.sendMessage(chat,{text:body,mentions:[sender]},{quoted:msg})}}}
