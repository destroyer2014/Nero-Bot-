...
function rawJids(value = {}) {
  return [value?.id, value?.jid, value?.lid, value?.phoneNumber, value?.participant, value?.participantAlt]
    .filter(Boolean)
    .map(item => String(item))
}
function jidKey(value = '') {
  return String(value).replace(/:\d+@/, '@').split('@')[0].replace(/\D/g, '')
}
function sameIdentity(a, b) {
  const left = jidKey(a)
  const right = jidKey(b)
  return Boolean(left && right && left === right)
}
function participantMatches(participant, candidates = []) {
  const values = rawJids(participant)
  return values.some(value => candidates.some(candidate => value === candidate || sameIdentity(value, candidate)))
}
function targetFromMsg(ctx){const m=ctx.msg.message||{};const c=m.extendedTextMessage?.contextInfo||m.imageMessage?.contextInfo||m.videoMessage?.contextInfo;return jidNormalizedUser(c?.mentionedJid?.[0]||c?.participantAlt||c?.participant||'')}
...
async function requireAdmin(ctx){
  ...
  const senderCandidates=[ctx.sender,ctx.msg?.key?.participant,ctx.msg?.key?.participantAlt,ctx.msg?.key?.remoteJidAlt].filter(Boolean)
  const p=m.participants.find(item=>participantMatches(item,senderCandidates))
...
async function requireBotAdmin(ctx){
  ...
  const botCandidates=[ctx.sock.user?.id,ctx.sock.user?.jid,ctx.sock.user?.lid].filter(Boolean)
  const bot=m.participants.find(item=>participantMatches(item,botCandidates))
  if(!bot?.admin){
    console.log('[ADMIN] Bot no reconocido como admin',{botCandidates,participants:m.participants.map(x=>({id:x.id,lid:x.lid,phoneNumber:x.phoneNumber,admin:x.admin}))})
    throw new Error('El bot necesita ser administrador.')
  }
  return m
}
...
export const promote=wrap('promote',['daradmin'],async ctx=>{await requireBotAdmin(ctx);const t=targetFromMsg(ctx);if(!t)throw new Error('Menciona o responde al usuario.');await ctx.sock.groupParticipantsUpdate(ctx.chat,[t],'promote')})
export const demote=wrap('demote',['quitaradmin'],async ctx=>{await requireBotAdmin(ctx);const t=targetFromMsg(ctx);if(!t)throw new Error('Menciona o responde al usuario.');await ctx.sock.groupParticipantsUpdate(ctx.chat,[t],'demote')})
export const kick=wrap('kick',['expulsar'],async ctx=>{await requireBotAdmin(ctx);const t=targetFromMsg(ctx);if(!t)throw new Error('Menciona o responde al usuario.');await ctx.sock.groupParticipantsUpdate(ctx.chat,[t],'remove')})
...
export const testwelcome=wrap('testwelcome',[],ctx=>sendGreetingPreview(ctx,'welcome'))
export const testgoodbye=wrap('testgoodbye',[],ctx=>sendGreetingPreview(ctx,'goodbye'))
