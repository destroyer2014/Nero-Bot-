...
const number = value => {
  ...
  return Number.isFinite(parsed) ? new Intl.NumberFormat('es-PE').format(parsed) : clean(value)
}
...
const date = value => {
  ...
  return Number.isNaN(parsed.getTime())
    ? clean(value)
    : parsed.toLocaleDateString('es-PE', { timeZone: config.timezone, year: 'numeric', month: 'long', day: 'numeric' })
}
...
const wrap = (name, aliases, execute) => ({
  ...
  async execute(ctx) {
    try {
      ...
      await ctx.sock.sendMessage(ctx.chat, { text: `❌ ${error?.message || 'No se pudo consultar la cuenta.'}` }, { quoted: ctx.msg })
    }
  }
})

function requireHandle(ctx, example, { allowSpaces = false } = {}) {
  const value = handle(ctx.args.join(allowSpaces ? ' ' : ''))
  if (!value) throw new Error(`Uso: ${example}`)
  if (!allowSpaces && /\s/.test(value)) throw new Error('Escribe únicamente el nombre de usuario, sin espacios.')
  if (value.length > 100) throw new Error('El nombre de usuario es demasiado largo.')
  return value
}
...
async function sendProfile(ctx, imageUrl, caption) {
  if (imageUrl && /^https?:\/\//i.test(imageUrl)) {
    try {
      await ctx.sock.sendMessage(ctx.chat, { image: { url: imageUrl }, caption }, { quoted: ctx.msg })
      return
    } catch (error) {
      console.warn('[STALKING] No se pudo enviar la imagen:', error?.message || error)
    }
  }
  await ctx.sock.sendMessage(ctx.chat, { text: caption }, { quoted: ctx.msg })
}

export const githubStalk = wrap('githubstalk', ['ghstalk', 'githubinfo'], async ctx => {
  const username = requireHandle(ctx, '.githubstalk <usuario>')
  const data = await evoGet('/stalking/github', { username })
  const result = data?.result
  if (!result?.username) throw new Error('GitHub no devolvió información para ese usuario.')

  const languages = Array.isArray(result.top_languages)
    ? result.top_languages.slice(0, 5).map(item => `${clean(item.language)} (${number(item.repos)} repos)`).join(', ')
    : 'No disponible'
  const repositories = Array.isArray(result.top_repos)
    ? result.top_repos.slice(0, 5).map((repo, index) => `${index + 1}. ${clean(repo.name)} • ⭐ ${number(repo.stars)} • 🍴 ${number(repo.forks)}${repo.language ? ` • ${repo.language}` : ''}`).join('\n')
    : 'No disponible'
...
export const instagramStalk = wrap('instagramstalk', ['instastalk', 'igstalk'], async ctx => {
  const username = requireHandle(ctx, '.instagramstalk <usuario>')
  const data = await evoGet('/stalking/instagram', { username })
  const result = data?.result
  if (!result?.username) throw new Error('Instagram no devolvió información para ese usuario.')
  const caption = [
    '🔎 *Instagram público*',
    '',
    `Usuario: @${clean(result.username)}`,
...
export const robloxStalk = wrap('robloxstalk', ['rbstalk', 'robloxinfo'], async ctx => {
  const username = requireHandle(ctx, '.robloxstalk <usuario>')
  const data = await evoGet('/stalking/roblox', { username })
  const result = data?.data
  if (!result?.account?.username) throw new Error('Roblox no devolvió información para ese usuario.')
  const account = result.account
  const presence = result.presence || {}
  const stats = result.stats || {}
  const badges = Array.isArray(result.badges) && result.badges.length
    ? result.badges.slice(0, 10).map(item => clean(item?.name || item)).join(', ')
    : 'Ninguna'
  const caption = [
    '🔎 *Roblox público*',
    '',
...
export const telegramStalk = wrap('telegramstalk', ['tgstalk', 'telegraminfo'], async ctx => {
  const channel = requireHandle(ctx, '.telegramstalk <usuario_del_canal>')
  const data = await evoGet('/stalking/telegram', { channel })
  const result = data?.data
  if (!result || (!result.title && !result.username && !result.subscribers && !(result.messages || []).length)) {
    throw new Error('Telegram no devolvió información. Usa el username público del canal, sin @ ni espacios.')
  }
  const messages = Array.isArray(result.messages) && result.messages.length
    ? result.messages.slice(0, 5).map((item, index) => `${index + 1}. ${clean(item?.text || item?.message || item)}`).join('\n')
    : 'No disponibles'
  const caption = [
    '🔎 *Telegram público*',
    '',
...
export const tiktokStalk = wrap('tiktokstalk', ['ttstalk', 'tiktokinfo'], async ctx => {
  const username = requireHandle(ctx, '.tiktokstalk <usuario>')
  const data = await evoGet('/stalking/tiktok', { username })
  const result = data?.result
  if (!result?.username) throw new Error('TikTok no devolvió información para ese usuario.')
  const videos = Array.isArray(result.recent_videos) && result.recent_videos.length
    ? result.recent_videos.slice(0, 5).map((item, index) => `${index + 1}. ${clean(item?.title || item?.description || item?.url || item)}`).join('\n')
    : 'No disponibles'
  const caption = [
    '🔎 *TikTok público*',
    '',
