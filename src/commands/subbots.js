...
export const codeCommand={name:'code',aliases:['jadibot'],async execute(ctx){
 const phone=num(ctx.sender); if(phone.length<8) throw new Error('No pude detectar tu número. Usa .code <número con código de país>.')
 ...
 const id=phone; const old=getSubbot(id); if(old?.status==='connected') throw new Error('Ya tienes un subbot conectado.')
 markCodeRequest(ctx.sender); await startSubbotProcess({id,phone,requestChat:ctx.chat,requester:ctx.sender})
 await ctx.sock.sendMessage(ctx.chat,{text:'⏳ *NERO está preparando tu código de vinculación.*\nPuede tardar unos segundos. Recibirás el código en este mismo chat.'},{quoted:ctx.msg})
}}
